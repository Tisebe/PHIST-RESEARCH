setwd('C:/Users/Tony Isebe/Documents/R/win-library/3.6/parathyroidSE/extdata')
library(parathyroidSE)
??mustWork


##Finding the external data

extdata <- system.file('extdata', package = 'parathyroidSE', mustWork = TRUE)

list.files(extdata)


###Creating a table with experimental metadata, and for the three files it is as follows;

sampleTable <- data.frame(
  sampleName=c('Ctrl_24h_1','Ctrl_48h_1','DPN_24h_1'),
  filename=c('SRR479054.bam','SRR479052.bam','SRR479053.bam'),
...
##constructing full paths to the files we want to perform the counting operation on

bamFiles <- file.path(extdata, sampleTable$filename)
bamFiles


##We can have a look at one of the BAM files to see the naming style of the sequences-chromosomes using the BamFile function from the Rsamtools package, hence loading the library
...
##To count the number of genes which map to a given gene, we need the transcript annotation which can be downloaded from the GTF file with the human gene annotation from Ensembl

##From this file the function MakeTrxDbfromGff from the Genomic Features package constructs a database of all annotated transcripts

##Hence, preparing the gene models from the Gff files

library(GenomicFeatures)

hse <- makeTxDbFromGFF('Homo_sapiens.GRCh37.75.subset.gtf', format = 'gtf')

##Extracting exons for each gene

exonsByGene <- exonsBy(hse, by='gene')
exonsByGene


##Then counting the number of reads the function SummarizeOverlaps from the GenomicAlignments will perform the operation
...
##Summarized Experiment Object: Output of Counting
...
##A bonus about the workflow we have shown above is that information about the gene models we used is included without extra effort. The str R function is used to compactly display the structure of the data in the list.
...
##Quickly check whether we have the right samples
...
##recalling the design we have specified

design(dds)

##Running DESEQ: Modeling counts with patient and treatment effects
...
plotDispEsts(dds, ylim=c(1e-6, 1e-1))
...
testCategory("109606")
##WORKING WITH R LOGTRANSFORMED DATA
...
##Sample Distances-measure of similarity between different samples
...
colours <- c(rgb(1:3/4,0,0),rgb(0,1:3/4,0),rgb(0,0,1:3/4))
